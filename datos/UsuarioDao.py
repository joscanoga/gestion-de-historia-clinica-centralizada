...
class UsuarioDao:
    ...
    def datosPacientes(cls,**data):
        cursor = Conexion.obtenerCursor()
        consulta = "SELECT nombre , email ,telefono, direccion, fecha_nacimiento FROM {} WHERE id= '{}'   ".format(data["tipo"], data["id"])
        cursor.execute(consulta)
        registros = cursor.fetchall()
        dato=registros[0]
        id=data["id"]
        datos='{"datos":{ "id":"'+id+'","nombre" : "'+dato[0]+'","email":"'+dato[1]+'","telefono":"'+dato[2]+'","direccion":"'+dato[3]+'", "fecha_nacimiento":"'+str(dato[4])+'"}}'
        return datos
...
print(UsuarioDao.datosMedico(id="11",tipo="medico"))